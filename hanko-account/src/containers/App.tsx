...
export class App extends React.Component<AppProps, AppState> {
  ...
  fetchDevicesAndEdit = () => {
    ...
    fetchApi(keycloak, '/hanko/devices').then(jsonArray => {
      ...
      if (devices.length >= 0) {
        const datesArray = devices.map(item => item.createdAt)
        const latestDate = datesArray.sort().reverse()[0]
        const latestIndex = devices
          .map(item => item.createdAt)
          .indexOf(latestDate)
        this.setState({ editIndex: latestIndex })
      }
    })
  }
  ...
  fetchDevices = () => {
    ...
    fetchApi(keycloak, '/hanko/devices').then(jsonArray => {
      const devices = (jsonArray as any[]).map(json => deviceFromJson(json))
      this.setState({ devices: devices, editIndex: undefined })
    })
  }
  ...
  updateEditIndex = (editIndex: number | undefined, shouldRefetch: boolean) => {
            ...
            <div className="container">
              ...
              {devices === undefined ? (
                ...
                <RegisteredDevices
                  ...
                  editIndexChangedHandler={this.updateEditIndex}
              ...
              {showAddHankoAuthenticator ? (
                ...
                <div className="button-list pull-right">
                  ...
                  >
                    {content => (
                      <AddWebAuthn
                        refetch={this.fetchDevicesAndEdit}
                        keycloak={keycloak}
                        type="roaming"
                      >
                        {content}
                  ...
                  {showAddPlatformAuthenticator ? (
                    <FormattedMessage
                      id="App.addPlatformAuthenticatorButton"
                      defaultMessage="Add Platform Authenticator"
                    >
                      {content => (
                        <AddWebAuthn
                          refetch={this.fetchDevicesAndEdit}
